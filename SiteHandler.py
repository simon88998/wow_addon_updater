...
def curse(addonpage):
    ...
    try:
        page = scrapper.get(addonpage + '/download')
...
def getCurseVersion(addonpage):
    ...
    try:
        page = scrapper.get(addonpage + '/files')
        page.raise_for_status()   # Raise an exception for HTTP errors
        contentString = str(page.content)
        ...
        print('Failed getCurseVersion(addonpage) version number for: ' + addonpage)
        return ''
...
def getCurseProjectVersion(addonpage):
    try:
        page = scrapper.get(addonpage + '/files')
        print('getCurseVersion page: ' + page)
        if page.status_code == 404:
            # Maybe the project page got moved to WowAce?
            page = scrapper.get(addonpage)
            page.raise_for_status() # Raise an exception for HTTP errors
            page = scrapper.get(page.url + '/files') # page.url refers to the place where the last one redirected to
            page.raise_for_status()   # Raise an exception for HTTP errors
        contentString = str(page.content)
...
def wowinterface(addonpage):
    ...
    try:
        page = scrapper.get(downloadpage + '/download')
        page.raise_for_status()   # Raise an exception for HTTP errors
        contentString = str(page.content)
